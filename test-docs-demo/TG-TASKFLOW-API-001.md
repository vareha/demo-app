...
## 1. Feature Context
### What We're Testing
RESTful API endpoints for task CRUD operations in TaskFlow project management system. Covers creating, reading, updating, and deleting tasks with proper validation, error handling, and data integrity. This version also introduces task assignment functionality.
...
- **RISK-005**: Data leakage by assigning tasks to users in other organizations → **Test Focus**: Verify organization checks when assigning tasks
...
**Priority**: P1  
**Prerequisites**: Authenticated user with valid JWT token  
**Affects**: Web dashboard task assignment feature  
...
```json
{
  ...
  "description": "Add functionality to assign tasks",
  "status": "TODO",
  "priority": "HIGH",
  "due_date": "2025-12-01",
...
**Verify**: 
- Task is successfully assigned to the specified user
- User exists in the same organization  

---

#### TEST-HP-008: Update Task Assignment
**Component**: Task API Service  
**Endpoint**: PUT /api/tasks/{id}  
**Feature Tags**: task-assignment, update, api-crud  
**Test Type**: Happy Path  
**Priority**: P1  
**Prerequisites**: Task exists, authenticated user has permission  
**Affects**: Web dashboard task assignment feature  

**Given**: Task with ID "abc123" exists and is assigned to "user-id-123"  
...
**Then**: 
- Returns 200 OK
- Task assignment is updated in database  
**Verify**: 
- Task is now assigned to "user-id-456"
- Audit log entry created for assignment change  
...
**Component**: Task API Service  
**Endpoint**: PUT /api/tasks/{id}  
**Feature Tags**: task-assignment, unassign, api-crud  
**Test Type**: Happy Path  
**Priority**: P1  
**Prerequisites**: Task exists, authenticated user has permission  
**Affects**: Web dashboard task assignment feature  

**Given**: Task with ID "abc123" is assigned to "user-id-123"  
**When**: PUT /api/tasks/abc123 with payload to unassign:  
```json
{
  "assigned_to": null
}
...
**Then**: 
- Returns 200 OK
- Task is unassigned in database  
**Verify**: 
- assigned_to field is set to null
- Audit log entry created for unassignment  
...
**Given**: Current date is 2025-11-07  
